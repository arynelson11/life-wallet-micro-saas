    ...
    return (
        <header className="mb-8 pt-4">
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                            ...
                            <div className="grid gap-4 py-4">
                                <div className="flex items-center gap-4 p-4 border rounded-xl hover:bg-accent cursor-pointer transition-colors">
                                    <div className="w-10 h-10 bg-primary/20 rounded-lg flex items-center justify-center text-primary"><LayoutGrid className="w-5 h-5" /></div>
                                    <div><h4 className="font-semibold">Métricas de Vendas</h4><p className="text-sm text-muted-foreground">Gráfico de área com KPI</p></div>
                                </div>
                            </div>
                        </DialogContent>
                    </Dialog>
                    <Popover>
                        ...
                        <PopoverContent className="w-80" align="end">
                            <div className="space-y-4">
                                <div className="flex justify-between items-center"><h4 className="font-medium leading-none">Notificações</h4></div>
                                {/* ... Notifications Map ... */}
                                <div className="grid gap-4 max-h-[300px] overflow-y-auto">
                                    {notifications.map(n => (
                                        <div key={n.id} className={`flex items-start gap-4 p-3 rounded-lg ${n.read ? 'opacity-50' : 'hover:bg-muted/50'}`}>
                                    ...
                                    <Button variant="ghost" size="sm" className="w-full text-xs" onClick={markAllAsRead}>Marcar todas como lidas</Button>
                                </div>
                            </div>
                        </PopoverContent>
                    </Popover>
                    <div className="flex items-center gap-3 pl-2">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-56">
                                ...
                                <DropdownMenuItem onClick={() => push('/perfil')}><User className="mr-2 h-4 w-4" /><span>Perfil</span></DropdownMenuItem>
                                <DropdownMenuItem onClick={() => push('/settings')}><Settings className="mr-2 h-4 w-4" /><span>Configurações</span></DropdownMenuItem>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={handleLogout} className="text-red-600"><LogOut className="mr-2 h-4 w-4" /><span>Sair</span></DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                    </div>
                </div>
            ...
            <div className="flex md:hidden items-center justify-between">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Avatar className="w-10 h-10 border border-white/10 shadow-sm cursor-pointer">
                            <AvatarImage src={user?.user_metadata?.avatar_url} />
                            <AvatarFallback>{user?.email?.substring(0, 2).toUpperCase()}</AvatarFallback>
                        </Avatar>
                    </DropdownMenuTrigger>
                ...
                {/* Title/Logo (Center/Left-ish usually, but simplified) */}
                <h1 className="text-lg font-bold tracking-tight">LifeWallet</h1>

                {/* Notifications + Search Icon (Right) */}
                <div className="flex items-center gap-2">
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button size="icon" variant="ghost" className="rounded-full w-10 h-10 relative">
                                <Bell className="w-5 h-5" />
                                {unreadCount > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full animate-pulse" />}
                            </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-80" align="end">
                            <div className="p-2 text-center text-sm text-muted-foreground">Sem novas notificações</div>
                        </PopoverContent>
                    </Popover>
                </div>
            </div>
